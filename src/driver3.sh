...
then
    ...
    if [ "{{shconfig_env_type}}" == "dev" ]
    then
	for i in apache/01general.conf wp/10wordpress.conf rt/90rt.conf
	do
	    conf_filename=${i##*/}
    ...
    then
	# FIXME put in whatever code you want to populate your web
	# server config. See above dev config as an example.
	:
...
then
    ...
    # This is forced because the DB grants can include references to
